"""Aggregates the 3-seed ExtraTrees bootstrap=True rerun (see run_phase2_bootstrap.ps1)
into the same summary format as run_all_seeds.py's baseline aggregation."""
import json
...
all_results = []
for seed in SEEDS:
    with open(RUNS / f"seed{seed}" / "bootstrap" / "phase2_results.json") as f:
...
with open(RUNS / "summary_3seeds_bootstrap.json", "w") as f:
    json.dump(summary, f, indent=2)

print("\n=== 3-seed ExtraTrees-bootstrap summary (mean +/- std, %) ===")
...
print(f"\nsaved: {RUNS / 'summary_3seeds_bootstrap.json'}")
